...
import { Keypair } from '@/lib/soroban';
import {
  createPasskey,
...
interface BrowserSupport {
  ...
  error?: string;
  // Detalles opcionales para depuración (userAgent, flags, etc.)
  details?: {
    webauthn?: boolean;
...
const AdvancedPasskeyDemo: React.FC<AdvancedPasskeyDemoProps> = ({ onSuccess, onBack }) => {
  ...
  const [bundlerKey, setBundlerKey] = useState<any | null>(null);
  ...
  const renderSetupStep = () => (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold mb-4 text-center" style={{ color: 'var(--accent-400)' }}>
          🚀 Demo Avanzado - Passkeys sobre Soroban
        </h2>
        <p className="text-center mb-6" style={{ color: 'var(--muted-300)' }}>
          Crea una cuenta blockchain sin contraseñas usando WebAuthn y Soroban
        </p>
      </div>
      ...
      <div>
        <label htmlFor="username" className="block text-sm font-medium mb-2" style={{ color: 'var(--muted-300)' }}>
          Nombre de usuario
        </label>
        <input
          id="username"
          ...
          className="w-full px-3 py-2 rounded-md bg-[rgba(255,255,255,0.02)] text-[var(--foreground)] focus:outline-none focus:ring-2"
          disabled={loading}
        />
      </div>
      ...
      <div className="flex space-x-3">
        ...
        <button
          ...
          className="flex-1 py-3 px-4 rounded-md text-black font-semibold transition-colors"
  ...
  return (
  <div className="relative min-h-screen bg-gradient-to-br from-blue-950 via-blue-800 to-blue-500 flex items-center justify-center p-4">
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="text-center mt-6 text-blue-200 text-sm">
          <p>Powered by Logitec, Soroban y WebAuthn</p>
        </div>
      </div>
